...
from finetuned_densenet_embedding_extractor import FineTunedDenseNetEmbeddingExtractor

ROI_ROOT = AI_ROOT / "outputs" / "roi_dataset_v3"
MODEL_PATH = AI_ROOT / "outputs" / "models" / "densenet_best.pth"
OUTPUT_DIR = AI_ROOT / "outputs" / "densenet_embeddings_v3"

def main():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")

    extractor = FineTunedDenseNetEmbeddingExtractor(
        MODEL_PATH,
        device=device,
        num_classes=600
    )
    ...
    print("DenseNet embeddings saved.")
