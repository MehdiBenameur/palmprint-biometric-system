...
EMB_DIR = AI_ROOT / "outputs" / "resnet_embeddings"
...
def main():
    ...
    top1_ids = gallery_ids[top1]
    top1_scores = scores[np.arange(len(query_ids)), top1]
    ...
    top3_acc = np.mean([query_ids[i] in gallery_ids[top3[i]] for i in range(len(query_ids))])
    top5_acc = np.mean([query_ids[i] in gallery_ids[top5[i]] for i in range(len(query_ids))])
    top10_acc = np.mean([query_ids[i] in gallery_ids[top10[i]] for i in range(len(query_ids))])
